...
#
# Replace DataFolder with this working directory path where data is stored
#
...
medianSummary <- aggregate(data=Summary,.~Condition,FUN=median)

# Add in "_sd" to labels for std summary data
names(stdSummary)[-c(1,2)] <- paste(names(stdSummary)[-c(1,2)],'_sd',sep='')

# Combine the average and sd data into one dataframe
...
for (p in seq(1,length(PlotsToBeMade),2)) {
  ...
  plot(ggplot(data=Summary_AVG_STD,aes_string(x=feature1, y=feature2, color="Condition")) + geom_point(size=5) +
         ...
         scale_color_manual(values = getPalette(nc), labels=TheLabels__) +
         theme_bw() +
  ...
  ggsave(paste(DataFolder,"Plot-",p,".eps",sep=""), width = 14, height = 16, units = "cm", dpi = 300)
...
ggplot(Summary_AVG_STD_long, aes(fill = Condition, x = variable, y = value)) +
  ...
  scale_fill_manual(values = getPalette(nc), labels=TheLabels__) +
...
ggsave(paste(DataFolder,"AVG_SD_metrics.eps",sep=""), width = 18, height = 16, units = "cm", dpi = 300)
...
ggplot(Summary_long, aes(fill=Condition, x=variable, y=value)) +
  ...
  scale_colour_manual(values=rep("grey20",nc), labels=TheLabels__) +
...
ggsave(paste(DataFolder,"All_metrics.eps",sep=""), width = 20, height = 16, units = "cm", dpi = 300)
...
ggsave(paste(DataFolder,"MitoGraph_Connectivity_score.eps",sep=""), width = 20, height = 16, units = "cm",
       dpi = 300)

write.table(Summary_AVG_STD_long, file = paste(DataFolder,"Summary_AVG_STD.csv",sep=""), sep = ",", quote = FALSE, row.names = F)
...
head(AOV_Stats_New)
write.csv(AOV_Stats_New, paste(DataFolder,"AOV_stats_new.csv",sep=""))
