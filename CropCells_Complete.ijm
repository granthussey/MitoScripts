...
// File.makeDirectory(Root + "Imaging");
// ImagingDir = Root + "Imaging";

//CropCells_GaussNoise(Root);
CropCellsFolder_NOGaussNoise(Root);
//CropCells_NOGaussNoise(Root);
CropCellsFolder_GaussNoise(Root);

function CropCells_GaussNoise(RootFolder) {
...
File.makeDirectory(_RootFolder + "Gauss_BatchCells");
...
for (roi = 0; roi < roiManager("count"); roi++) {
	...
	save(_RootFolder + "Gauss_BatchCells/" + _FileName + "_" + IJ.pad(roi,3) + ".tif");
...
function CropCells_NOGaussNoise(RootFolder) {
...
// Creating a directory where the files are saved
File.makeDirectory(_RootFolder + "NoGauss_BatchCells");
...
for (roi = 0; roi < roiManager("count"); roi++) {
	...
	save(_RootFolder + "NoGauss_BatchCells/" + _FileName + "_" + IJ.pad(roi,3) + ".tif");
